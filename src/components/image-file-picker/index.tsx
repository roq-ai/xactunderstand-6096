...
import { FileUpload, useRoqFileUploader, useSession } from '@roq/nextjs';
import { FileInterface } from '@roq/ui-react/dist/features';
import { createFileAssociation, deleteFileAssociation } from 'apiSdk/files';
...
export const ImagePicker: FC<ImagePickerComponentProps> = forwardRef(function ImagePickerComponent(inputProps, ref) {
  ...
  const handleDeleteFile = async () => {
    ...
    if (filesQuery?.files?.data?.[0]) {
      await deleteFileAssociation(filesQuery?.files?.data?.[0].id);
    }
    mutate();
  };
  ...
  const handleUpload = useCallback(async () => {
    if (selectedImage) {
      ...
      await createFileAssociation({
        ...
        entity,
        fileId: file.fileId,
        ...(filesQuery?.files?.data?.length && {
          oldFileId: filesQuery?.files?.data?.[0].id,
        }),
      });
    }
